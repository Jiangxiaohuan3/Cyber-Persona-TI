...
export default async (request) => {
  ...
  try {
    ...
    if (payload?.kind === "start") {
      await updateSummarySnapshot(store, (summary) => addStartToSummary(summary, recordedAt));
      await store.setJSON(`starts/${now}-${id}.json`, {
        kind: "start",
        path: typeof payload.path === "string" ? payload.path : "/",
        questionCount: Number.isFinite(payload.questionCount) ? payload.questionCount : null,
        recordedAt
      });
    ...
    if (payload?.kind === "result") {
      ...
      await store.setJSON(`results/${encodeURIComponent(personaCode)}/${now}-${id}.json`, {
        kind: "result",
        personaCode,
        personaAlias: typeof payload.personaAlias === "string" ? payload.personaAlias : "",
